...
export default function Show({ page, isLiked, categories, posts }) {
          ...
          {Object.entries(categories).map(([group, cats]) => (
            <div key={group} className="mb-2">
              ...
              <ul className="list-disc list-inside text-sm text-gray-600">
                {cats.map((c) => (
                  <li key={c}>
                    <Link href={route('user_pages.category', c.toLowerCase().replace(/\s+/g, '-'))} className="text-blue-600 hover:underline">
                      {c}
                    </Link>
                  </li>
                ))}
